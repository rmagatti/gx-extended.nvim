...
-- Track the order patterns were registered
local pattern_order = {}
...
local function run_match_to_urls()
    ...
    -- FIXED: Use ipairs over pattern_order instead of pairs over registry
    -- This ensures patterns are checked in registration order
    for _, file_glob in ipairs(pattern_order) do
...
function M.register(options)
    ...
    for _, pattern in ipairs(patterns) do
        if not registry[pattern] then
            registry[pattern] = {}
            -- Track when this pattern was first registered
            table.insert(pattern_order, pattern)
        end
